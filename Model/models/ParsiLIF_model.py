...
class ParsiLIFModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        # Overlay → masks
        self.overlay = self.real_B_array[4].to(self.device)
        pos, semantic = masks_from_overlay_torch(self.overlay)
    ...
    def backward_D(self):
        ...
        # Gradient clipping
        if hasattr(self.opt, 'clip_grad_norm') and self.opt.clip_grad_norm > 0:
            torch.nn.utils.clip_grad_norm_(
                itertools.chain(self.netD1.parameters(), self.netD2.parameters(),
                                self.netD3.parameters(), self.netD4.parameters()),
        ...
        if hasattr(self.opt, 'clip_grad_norm') and self.opt.clip_grad_norm > 0:
            torch.nn.utils.clip_grad_norm_(
                itertools.chain(self.netG1.parameters(), self.netG2.parameters(),
                                self.netG3.parameters(), self.netG4.parameters()),
                self.opt.clip_grad_norm
            )
    ...
    def gate_constraints_router(self, outputs):
        ...
        if gate_type in ('beta', 'betagate'):
            ...
            total = ig('total', None)
            terms.update({'total': total, 'gate_type': torch.tensor(3)})
